      ...
      <div class="bg-(--bg-elevated) border border-(--border) rounded-xl p-8 mb-8 flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          <div class="shrink-0">
              <div class="w-32 h-32 rounded-full border-4 border-(--primary) overflow-hidden shadow-[0_0_20px_var(--primary-glow)]">
                  ...
                  <img src="<?= $src ?>" alt="<?= esc($user["name"]) ?>" class="w-full h-full object-cover">
              </div>
          </div>

        <!-- Info -->
        <div class="flex-1 text-center md:text-left">
            <h1 class="h1 uppercase text-white mb-1"><?= esc($user["name"]) ?></h1>
            <p class="text-(--text-secondary) mb-4"><?= esc($user["email"]) ?></p>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
              ...
              <div class="bg-(--bg-elevated) border border-(--border) rounded-lg overflow-hidden group relative">
                  <a href="/movies/<?= esc($item["movie_slug"]) ?>" class="block relative aspect-2/3 overflow-hidden">
                      <img src="<?= $item["movie_poster"] ? esc($item["movie_poster"]) : "https://placehold.co/600x900/1a1a24/606078?font=oswald&text=Not+Found" ?>" alt="<?= esc($item["movie_title"]) ?>" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
                  ...
                  <div class="p-3 bg-(--bg-card) border-t border-(--border)">
                      <h3 class="font-bold text-white text-sm line-clamp-1 mb-2" title="<?= esc($item["movie_title"]) ?>"><?= esc($item["movie_title"]) ?></h3>
                      <div class="flex justify-between items-center">
                          <form action="/watchlist/<?= $item["id"] ?>/status" method="POST" class="inline" onsubmit="this.querySelector('button[type=submit]').disabled=true; this.querySelector('button[type=submit]').innerHTML='...';">
                              <input type="hidden" name="status" value="<?= $item["status"] === "watched" ? "watching" : "watched" ?>">
                              <?= csrf_field() ?>
                              <button type="submit" class="text-xs px-2 py-1 rounded <?= $item["status"] === "watched" ? "bg-green-900/50 text-green-400 border border-green-500/50" : "bg-blue-900/50 text-blue-400 border border-blue-500/50" ?>" title="Ubah Status">
                                  <?= $item["status"] === "watched" ? "Selesai" : "Menunggu" ?>
                              </button>
                          </form>
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2 mb-3">
                            <div class="rating-badge bg-(--primary) text-white px-2 py-0.5 rounded text-sm font-bold">★ <?= esc($review["rating"]) ?>/10</div>
                            <span class="text-xs text-(--text-muted)"><?= date("d M Y", strtotime($review["created_at"])) ?></span>
                        </div>
                        <h4 class="font-bold text-(--text-primary) mb-1"><?= esc($review["title"]) ?></h4>
            ...
            <dialog id="edit-review-modal-<?= $review["id"] ?>" class="bg-(--bg-card) text-white p-6 rounded-xl border border-(--border) shadow-2xl backdrop:bg-black/80 backdrop:backdrop-blur-sm w-[90%] max-w-lg m-auto">
                ...
                <form action="/reviews/<?= $review["id"] ?>/update" method="POST" onsubmit="this.querySelector('button[type=submit]').disabled=true; this.querySelector('button[type=submit]').innerHTML='Menyimpan...';">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="rating" min="1" max="10" value="<?= esc($review["rating"]) ?>" required class="w-full bg-(--bg-elevated) border border-(--border) rounded px-3 py-2 text-white focus:outline-none focus:border-(--primary)">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm mb-1 text-(--text-secondary)">Judul Ulasan</label>
                        <input type="text" name="title" value="<?= esc($review["title"]) ?>" required class="w-full bg-(--bg-elevated) border border-(--border) rounded px-3 py-2 text-white focus:outline-none focus:border-(--primary)">
                    </div>
                    <div class="mb-4">
